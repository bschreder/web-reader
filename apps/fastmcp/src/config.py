...
if _playwright_parsed.scheme not in ("ws", "wss"):
    raise RuntimeError("PLAYWRIGHT_WS_URL must use ws:// or wss://")
if not _playwright_parsed.hostname or not _playwright_parsed.port:
    raise RuntimeError("PLAYWRIGHT_WS_URL must include host and port (e.g., ws://playwright:3002)")
PLAYWRIGHT_HOST: Final[str] = _playwright_parsed.hostname
PLAYWRIGHT_PORT: Final[int] = _playwright_parsed.port
